...
class ModelWrapper(nn.Module):
        ...
        for _ in range(epochs):
            ...
            for i in range(0, len(all_data), batch_size):
                ...
                if config['conver_bound']:
                    ...
                    pdb.set_trace()
                    loss = utils.entropy_from_logits(logits)
    ...
    def layernorm_adaptation_additional_learn(self, config, dataset, save_dir=None, run_name=None):
        ...
        if config['post_attention']:
            ...
            patch_layernorm_with_rescaled_by_name(peft_model, alpha=0.1, trainable_alpha=False, match_key="input_layernorm", mode=config['additional_layernorm_mode'])
